...
#define BLE_UUID_HM10_CHARACTERISTIC 0xffe1                      /**< The UUID of the RX Characteristic. */
...
static void on_write(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
       ...
       )
    {
        ...
        {
            NRF_LOG_DEBUG("notification for hm10 enabled\n");
            p_nus->is_notification_enabled = true;
        }
        else
        {
            NRF_LOG_DEBUG("notification for hm10 disabled\n");
            p_nus->is_notification_enabled = false;
        }
    }else if (
        (p_evt_write->handle == nus_nrf.rx_handles.cccd_handle)
        &&
        (p_evt_write->len == 2)
       )
    {
        if (ble_srv_is_notification_enabled(p_evt_write->data))
        {
            NRF_LOG_DEBUG("notification for nrf enabled\n");
            nus_nrf.is_notification_enabled = true;
        }
        else
        {
            NRF_LOG_DEBUG("notification for nrf disabled\n");
            nus_nrf.is_notification_enabled = false;
        }
    }
    else if (
             (p_evt_write->handle == p_nus->rx_handles.value_handle || p_evt_write->handle == nus_nrf.tx_handles.value_handle)
             &&
...
static uint32_t char_add_hm10(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init, bool secure)
{
    ...
    ble_uuid.uuid = BLE_UUID_HM10_CHARACTERISTIC;
...
static uint32_t rx_char_add_nrf(bool secure)
{
    ...
    ble_uuid.type = nus_nrf.uuid_type;
    ...
    return sd_ble_gatts_characteristic_add(nus_nrf.service_handle,
                                           &char_md,
                                           &attr_char_value,
                                           &nus_nrf.rx_handles);
    /**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init, bool secure)
{
    ...
    ble_uuid.type = p_nus->uuid_type;
    ble_uuid.uuid = BLE_UUID_NUS_SERVICE_HM10;
    ...
    // NRF_LOG_DEBUG("tx_char_add add result: %d\n", err_code);
    // VERIFY_SUCCESS(err_code);

    ble_uuid_t    ble_uuid_nrf;
    ble_uuid128_t nus_base_uuid_nrf = NUS_BASE_UUID;

    VERIFY_PARAM_NOT_NULL(p_nus_init);

    // Initialize the service structure.
    nus_nrf.conn_handle             = BLE_CONN_HANDLE_INVALID;
    nus_nrf.data_handler            = p_nus_init->data_handler;
    nus_nrf.is_notification_enabled = false;

    /**@snippet [Adding proprietary Service to S110 SoftDevice] */
    // Add a custom base UUID.

    err_code = sd_ble_uuid_vs_add(&nus_base_uuid_nrf, &nus_nrf.uuid_type);
    VERIFY_SUCCESS(err_code);

    ble_uuid_nrf.type = nus_nrf.uuid_type;
    ble_uuid_nrf.uuid = BLE_UUID_NUS_SERVICE_NRF;


    // Add the service.
    err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY,
                                        &ble_uuid_nrf,
                                        &nus_nrf.service_handle);
    /**@snippet [Adding proprietary Service to S110 SoftDevice] */
    VERIFY_SUCCESS(err_code);
...
uint32_t ble_nus_string_send(ble_nus_t * p_nus, uint8_t * p_string, uint16_t length)
{
    ...
    if(nus_nrf.is_notification_enabled){
        hvx_params.handle = nus_nrf.rx_handles.value_handle;
        sd_ble_gatts_hvx(p_nus->conn_handle, &hvx_params);
    }
    return NRF_SUCCESS;
}
